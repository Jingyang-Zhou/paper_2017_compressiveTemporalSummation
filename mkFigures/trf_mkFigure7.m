...
b     = load(fullfile(dLoc, 'fMRI', dName));
...
for k = 1 : nroi
    ...
    sr_double{2}(k, :) = prctile(stn.derivedParam.r_double(idx1), [40, 60]);
    st_isi{2}(k, :) = prctile(stn.derivedParam.t_isi(idx1), [40, 60]);
end

% median and central derived parameters
for k = 1 : nroi
...
subplot(1, 2, 2)
plot(1 : nroi, mt_isi{1}, 'ko', 'markersize', 8, 'markerfacecolor', 'k'), hold on , 
plot([1 : nroi] + 0.3, mt_isi{2}, 'ro', 'markersize', 8, 'markerfacecolor', 'r')
for k = 1 : nroi
